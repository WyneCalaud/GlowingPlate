...
func _ready() -> void:
	...
	if not is_instance_valid(GD):
		push_error("LobbyCanteen: GameData not found!")
		return

	# 🔥 Always reset phase when entering lobby
	if "current_phase" in GD and "day_started" in GD:
	...
	if is_instance_valid(day_number) and "current_day" in GD:
		day_number.text = "Day %02d" % GD.current_day

	# --- AUTO START LOGIC ---
	if "auto_start_day" in GD and GD.auto_start_day:
		GD.auto_start_day = false # Consume the flag so it only happens once
		
		# Immediately hide lobby UI so the player never sees it
		if is_instance_valid(almanac_btn): almanac_btn.hide()
		if is_instance_valid(bulletin_btn): bulletin_btn.hide()
		if is_instance_valid(start_day_btn): start_day_btn.hide()
		if is_instance_valid(nutridesk_btn): nutridesk_btn.hide()
		if is_instance_valid(nutrishop_btn): nutrishop_btn.hide()
		if is_instance_valid(hideshowbutton): hideshowbutton.hide()
		if is_instance_valid(bottom_buttons): bottom_buttons.hide()
...
func auto_start_after_news():
	...
	if "day_started" in GD and GD.day_started:
		return
...
func _play_dialogue(text_content: String, box_size_type: String = "medium"):
	if not is_instance_valid(dialogue_box): return
...
func play_food_intro_if_needed():
	...
	if "shown_food_intros" in GD and GD.shown_food_intros.get(day, false):
		start_real_day()
		return
...
func spawn_principal_intro(day:int, text:String):
	var GD = get_node_or_null("/root/GameData")
	if is_instance_valid(GD) and "service_state" in GD:
		GD.service_state = GameData.ServiceState.IDLE
	...
	if is_instance_valid(customer_manager):
...
func _restore_day_ui_state() -> void:
	...
	if "day_started" in gd and gd.day_started:
		...
		if is_instance_valid(nutrishop_btn): nutrishop_btn.hide()
		if is_instance_valid(hideshowbutton): hideshowbutton.hide()
		if is_instance_valid(bottom_buttons): bottom_buttons.hide() # <-- ADDED THIS
	else:
		is_waiting_for_serve = false

		if is_instance_valid(almanac_btn): almanac_btn.show()
		if is_instance_valid(bulletin_btn): bulletin_btn.show()
		if is_instance_valid(start_day_btn): start_day_btn.show()
		if is_instance_valid(nutridesk_btn): nutridesk_btn.show()
		if is_instance_valid(nutrishop_btn): nutrishop_btn.show()
		if is_instance_valid(hideshowbutton): hideshowbutton.show()
		if is_instance_valid(bottom_buttons): bottom_buttons.show() # <-- ADDED THIS
...
func _check_for_returned_items() -> void:
	if OrderSystem.prepared_plate_contents.size() > 0:
		...
		if is_instance_valid(final_plate_display):
	...
	if OrderSystem.prepared_beverage_data.size() > 0:
		...
		if is_instance_valid(final_beverage_display):
...
func _debug_plate_slots() -> void:
	...
	var expected: Dictionary = OrderSystem.current_customer_order.required_plate if OrderSystem.current_customer_order else {}
...
func show_final_plate(contents: Array) -> void:
	...
	if has_node("FinalPlateDisplay/PlateDisplay"):
		$FinalPlateDisplay/PlateDisplay.texture = preload("res://Items/Plates/Plate Final.png")

	var sprite_map = {}
	if has_node("FinalPlateDisplay/PlateSpriteGo"): sprite_map["Go"] = $FinalPlateDisplay/PlateSpriteGo
	if has_node("FinalPlateDisplay/PlateSpriteGrow"): sprite_map["Grow"] = $FinalPlateDisplay/PlateSpriteGrow
	if has_node("FinalPlateDisplay/PlateSpriteVeg"): sprite_map["GlowVeg"] = $FinalPlateDisplay/PlateSpriteVeg
	if has_node("FinalPlateDisplay/PlateSpriteFru"): sprite_map["GlowFru"] = $FinalPlateDisplay/PlateSpriteFru
	...
	for entry in contents:
		...
		if is_instance_valid(sprite) and tex:
			sprite.texture = tex
...
func show_final_beverages(beverage_data: Dictionary) -> void:
	...
	var sprites = []
	if has_node("FinalBeverageDisplay/BeverageSprite1"): sprites.append($FinalBeverageDisplay/BeverageSprite1)
	if has_node("FinalBeverageDisplay/BeverageSprite2"): sprites.append($FinalBeverageDisplay/BeverageSprite2)
	...
	for entry in beverage_data.values():
		...
		var tex = _get_item_texture(entry)
		if tex and is_instance_valid(sprites[index]):
...
func _on_btn_final_serve_pressed() -> void:
	if has_node("FinalPlateDisplay/BtnFinalServe"):
	...
	var character_id := ""
	if "saved_customer_order" in GD and GD.saved_customer_order:
	...
	if correct:
		...
		if character_id != "" and is_instance_valid(customer_manager):
			customer_manager.play_sad_reaction(character_id)

	# NOW finalize service
	if GD.has_method("finalize_service"):
		GD.finalize_service({
	...
	if "service_state" in GD: GD.service_state = GameData.ServiceState.SERVED
	if "returning_from_kitchen" in GD: GD.returning_from_kitchen = false

	current_happiness = 100.0

	if is_instance_valid(final_beverage_display): final_beverage_display.hide()
	if is_instance_valid(final_plate_display): final_plate_display.hide()

	if is_instance_valid(dialogue_box): dialogue_box.show()
	
	if has_node("DayScene/BtnAccept"): $DayScene/BtnAccept.hide()
	if has_node("DayScene/BtnContinue"): $DayScene/BtnContinue.hide()

	# If this was the LAST customer, stop here.
	if "remaining_customers" in GD and GD.remaining_customers.is_empty():
		return
	...
	if "remaining_customers" in GD and not GD.remaining_customers.is_empty():
		spawn_next_customer()
...
func _get_detailed_mistakes() -> Array:
	...
	var required_plate: Dictionary = OrderSystem.current_customer_order.required_plate if OrderSystem.current_customer_order else {}
	var required_portions: Dictionary = OrderSystem.current_customer_order.get("required_portions", {}) if OrderSystem.current_customer_order else {}
	var required_beverages: Array = OrderSystem.current_customer_order.get("required_beverage", []) if OrderSystem.current_customer_order else []
	...
	if prepared_beverages.size() == 0 and required_beverages.size() > 0:
...
func _on_day_button_pressed() -> void:
	...
	match GD.current_day if "current_day" in GD else 1:
	...
	if "day_started" in GD and not GD.day_started:
		if GD.has_method("start_day_with_orders"):
			GD.start_day_with_orders(day_orders)

	# -------------------------
	# DAY 1 → No mini game
	# -------------------------
	if "current_day" in GD and GD.current_day == 1:
		play_food_intro_if_needed()
		_restore_day_ui_state()
		return
	...
	# -------------------------
	if "current_phase" in GD and GD.current_phase == GD.GamePhase.NEWS:
		GD.current_phase = GD.GamePhase.LOBBY
		play_food_intro_if_needed()
		_restore_day_ui_state()
		return
...
func _emit_customer_exit():
	...
	if is_instance_valid(manager) and manager.current_customer:
		if manager.has_method("customer_leave"):
...
func _start_transition(type: String):
	...
	if has_node("Fade_transition"):
		...
		if has_node("Fade_transition/Fade_timer"):
...
func _on_glow_desk_pressed() -> void:
	if current_open_popup == glow_desk_ui:
		...
		if is_instance_valid(glow_desk_ui):
			open_popup(glow_desk_ui)

func open_popup(popup_node: Control) -> void:
	if not is_instance_valid(popup_node): return
	...
	if is_instance_valid(darken_overlay):
		darken_overlay.show()
...
func close_popup() -> void:
	if current_open_popup == null or not is_instance_valid(current_open_popup):
		return
...
func _on_almanac_pressed() -> void:
	if current_open_popup == almanac_ui:
		...
		if is_instance_valid(almanac_ui):
...
func _on_glow_board_pressed() -> void:
	if current_open_popup == glowboard:
		...
		if is_instance_valid(glowboard):
...
func _on_glow_shop_pressed() -> void:
	if current_open_popup == nutrishop_ui:
		...
		if is_instance_valid(nutrishop_ui):
			open_popup(nutrishop_ui)

func get_hud_patience() -> float:
	var hud = get_tree().get_first_node_in_group("HUD")
	if is_instance_valid(hud) and "patience" in hud:
		return hud.patience
	return 100.0
...
func _restore_patience_ui():
	...
	if not is_instance_valid(hud) or not is_instance_valid(GD):
		return

	# 🚫 If day isn't active, hide patience UI
	if "day_started" in GD and not GD.day_started:
		if hud.has_method("reset_patience"):
			hud.reset_patience()
		return

	# ✅ If a customer still exists, show paused patience
	if "saved_customer_order" in GD and GD.saved_customer_order != null:
		if hud.has_method("stop_patience"):
			hud.stop_patience()
...
func spawn_next_customer():
	...
	# ------------------------------------------------
	if "returning_from_kitchen" in GD and GD.returning_from_kitchen:
		return
	...
	if is_instance_valid(customer_manager) and customer_manager.is_animating:
		await get_tree().process_frame
		spawn_next_customer()
		return

	if "customer_patience" in GD: GD.customer_patience = 100.0
	if "patience_running" in GD: GD.patience_running = false
	...
	var tex: Texture2D
	var stage : int = GD.get_character_stage(order.customer_name) if GD.has_method("get_character_stage") else 1
	...
	if GD.has_method("save_customer"):
...
func _show_special_key_reward():
	...
	if is_instance_valid(GD) and "keys" in GD:
...
# DEFENSIVE FIX: Wipes current gameplay loop, flags the skipped day as completed, and bridges right to results
func _on_skip_day_pressed() -> void:
	...
	if is_instance_valid(GD):
		...
		# 🔥 DEFENSIVE FIX: Forcefully increment the day ONLY when skipping.
		# By doing it here, we ensure normal gameplay doesn't get double incremented.
		if "current_day" in GD:
			GD.current_day += 1
			if GD.has_method("save_game"):
				GD.save_game()

	# Safely transition to Results 
